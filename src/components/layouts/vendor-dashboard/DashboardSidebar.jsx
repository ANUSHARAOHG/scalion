...
const DashboardSidebar = props => {
  ...
  const content = <Scrollbar autoHide clickOnTrack={false} sx={{
    overflowX: "hidden",
  ...
  if (downLg) {
    return <LayoutDrawer open={showMobileSideBar ? true : false} onClose={setShowMobileSideBar}>
        <Box p={2} maxHeight={TOP_HEADER_AREA}>
          <Image alt="Logo" width={105} height={50} src="/assets/images/scalionlogo.svg" style={{
          marginLeft: 8
        }} />
        </Box>
  ...
  return <SidebarWrapper compact={sidebarCompact ? 1 : 0} onMouseEnter={() => setOnHover(true)} onMouseLeave={() => sidebarCompact && setOnHover(false)}>
      <FlexBetween p={2} maxHeight={TOP_HEADER_AREA} justifyContent={COMPACT ? "center" : "space-between"}>
        <Avatar src={COMPACT ? "/assets/images/circle.png" : "/assets/images/scalionlogo.svg"} sx={{
        borderRadius: 0,
        width: "auto",
